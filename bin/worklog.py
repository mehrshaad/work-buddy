...
# ------------------------------------------------------------------- teams ----
# Deliver the day's "Work update" section into a Teams chat. Optional and off by
# default: it needs a Power Automate flow you create yourself. docs/teams-daily-summary.md
# is the build guide, including why the obvious routes do not work.
#
# Review-first on purpose. `prepare` gathers, repairs and stages; only `send` posts, so
# a degraded summary never reaches a manager unread.
TEAMS_DEFAULTS = {"enabled": False,
                  "flow_url_file": "~/.worklog/teams_url",
                  "recipient_file": "~/.worklog/teams_recipient",
...
def teams_cfg(cfg: dict) -> dict:
    return {**TEAMS_DEFAULTS, **(cfg.get("teams") or {})}


def teams_staged(cfg: dict, day: date_cls) -> Path:
    d = expand(teams_cfg(cfg)["staging_dir"])
...
def teams_bullets(cfg: dict, day: date_cls):
    """The manager-facing part of the log: the Work update section, nothing after it."""
    md = expand(cfg["output_dir"]) / f"{day.isoformat()}.md"
    if not md.is_file():
        return None, f"no log written for {day}"
    text = md.read_text(encoding="utf-8")
    i = text.find("**Work update")
    if i < 0:
        return None, f"{md.name} has no Work update section"
    body = text[i:]
    end = body.find("\n---")               # everything before the Evidence block
    if end > 0:
        body = body[:end]
    # the reconstruction note is provenance for us, not something a manager needs
...
def teams_message(cfg: dict, text: str, html: bool = False) -> str:
    """HTML for the flow, which drops it into a message body; plain for a compose box."""
    t = teams_cfg(cfg)
    out = []
    for line in text.splitlines():
        ...
        if not s:
            ...
            out.append("\u2022 " + (_teams_html(s[2:]) if html else s[2:].replace("**", "")))
        else:
            out.append(_teams_html(s) if html else s.replace("**", ""))
    while out and not out[0]:
        out.pop(0)
    while out and not out[-1]:
        out.pop()
    if html:
        return ("<br>".join(out) + "<br><br>\u2014<br>Generated by "
                f"<a href=\"{t['footer_url']}\">{t['footer_name']}</a>")
    # a compose box sends markup literally, so the name cannot be a link there
    return "\n".join(out) + f"\n\n\u2014\nGenerated by {t['footer_name']} \u00b7 {t['footer_url']}"
...
def teams_prepare(cfg: dict, day: date_cls, quiet: bool = False) -> int:
    ...
    try:
        ...
        added = apply_addendum(cfg, date_cls.today())
        if added:
            log_line(cfg, f"teams: applied the after-hours addendum - {added}")
    except Exception as exc:
        log_line(cfg, f"teams: addendum failed {exc!r}")

    status, reason = report_health(expand(cfg["output_dir"]) / f"{day.isoformat()}.md")
    if status != "ok":
        log_line(cfg, f"teams: {day} is {status} ({reason}) - repairing before staging")
        ...
        # only re-summarize when the addendum actually added something, otherwise this
        # spends a model call every morning to reproduce the same bullets
        log_line(cfg, "teams: re-summarizing so the bullets cover last night's work")
        repair_day(cfg, day)
...
def teams_send(cfg: dict, day: date_cls, dry_run: bool = False) -> int:
    ...
    if not p.is_file():
        print(f"nothing staged for {day} - run `worklog teams prepare` first")
        return 1
    rec = json.loads(p.read_text())

    who = teams_read(t["recipient_file"])
    url = teams_read(t["flow_url_file"])
    if not (who or url):
        print(f"no destination yet - put a flow URL in {t['flow_url_file']} "
              f"or a Teams address in {t['recipient_file']}")
        return 1
    body = teams_message(cfg, rec["text"], html=bool(url))

    if dry_run:
        print(f"to: {who or '(the chat the flow targets)'}\n"
              f"via: {'Power Automate flow' if url else 'Teams deeplink (you press Enter)'}\n"
              f"chars: {len(body)}\n--- message ---\n{body}")
        return 0

    if url:
        req = urllib.request.Request(
            url, data=json.dumps({"date": rec["date"], "text": body},
        ...
        try:
            ...
            # a 202 only means the flow accepted it; the run history is the real proof
            print(f"sent via the flow (HTTP {code}) - confirm the run succeeded")
            return 0
        except Exception as exc:
            detail = exc.read()[:200] if isinstance(exc, urllib.error.HTTPError) else exc
            log_line(cfg, f"teams: flow POST failed ({detail!r}) - falling back to the deeplink")
            print(f"flow failed: {detail} - opening Teams instead")
    ...
    subprocess.run(["/usr/bin/pbcopy"], input=body, text=True, check=False)
    prefill = body if len(body) <= int(t["deeplink_max_chars"]) else ""
    link = (f"msteams:/l/chat/0/0?users={urllib.parse.quote(who)}"
    ...
    notify(cfg, "Summary ready in Teams",
           "Press Enter to send." if prefill else "Too long to pre-fill - paste with cmd-V.",
           subtitle=f"chat with {who}")
    print("opened Teams" + ("" if prefill else " (paste from the clipboard)"))
    return 0


def teams_state(cfg: dict, day: date_cls) -> dict:
    """One dict for the menu bar."""
    t = teams_cfg(cfg)
    p = teams_staged(cfg, day)
...
def cmd_teams(cfg: dict, args) -> int:
    ...
    if args.action == "show":
        ...
        return 0
    print(json.dumps(teams_state(cfg, day)))
    return 0
